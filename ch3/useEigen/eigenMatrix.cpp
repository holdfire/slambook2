...
int main(int argc, char** argv){
    //Eigen中所有向量和矩阵都是 Eigen::Matrix, 它是一个模板类，它的前三个参数为：数据类型，行，列
    //声明一个2 * 3的float矩阵
    Matrix<float, 2, 3> matrix_23;

    //同时，Eigen通过typedef提供了许多内置类型，不过底层仍是Eigen::Matrix
    //例如：Vector3d实质上是 Eigen::Matrix<double, 3, 1>，即三维向量
    Vector3d v_3d;
    //这是一样的
    ...
    // 但是在Eigen里，你不能混合两种不同类型的矩阵，像这样是错误的
   ...
   cout<<"[1,2,3;4,5,6] * [3;2;1] = "<<result.transpose()<<endl;
    ...
    cout<<"times 10:"<<10 * matrix_33<<endl;                 //数乘
